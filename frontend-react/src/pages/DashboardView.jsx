    ...
    <>
      ...
      <motion.div variants={container} initial="hidden" animate="visible">
        ...
        <div className="dv-grid">
          ...
          <div className="dv-main-col">
            {/* Top row: Upload + Scores side by side */}
            ...
            {/* Skill Gap — flows directly below the top row, no gap */}
            {hasResults && (
            ...
            {/* Key Resume Metrics — shown below skill gap when results exist */}
            {hasResults && (
              <div className="dv-metrics-row">
                <motion.div variants={item}>
                ...
                {/* Quick Stats */}
                <motion.div variants={item}>
                  <div className="dv-card" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    <h3 className="dv-metrics-title">Quick Stats</h3>
                    {[
                      { label: 'Skills Detected', value: resumeSkills.length, color: 'var(--accent)' },
                      { label: 'Matched Skills', value: matched.length, color: 'var(--success)' },
                      { label: 'Gaps to Close', value: missing.length, color: 'var(--warning)' },
                    ].map((s, i) => (
                      <div key={i} style={{
                        ...
                        <span style={{ fontSize: '0.85rem', color: 'var(--txt-3)', fontWeight: 600 }}>{s.label}</span>
                        <span style={{ fontFamily: 'var(--font-head)', fontWeight: 900, fontSize: '1.1rem', color: s.color }}>{s.value}</span>
                      </div>
                    ))}
                  </div>
                </motion.div>
          ...
          <div className="dv-right-col">
            ...
            {/* Key Resume Metrics (sidebar version — shown when no results) */}
            {!hasResults && (
              <motion.div variants={item}>
                <div className="dv-card dv-metrics-card">
                  <h3 className="dv-metrics-title">Key Resume Metrics</h3>
